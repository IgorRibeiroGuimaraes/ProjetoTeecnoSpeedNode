 ...
 */
export async function updateVanLetterService(id: number, data: any) {
  try {
    // Verifica se o registro existe antes de tentar atualizar
    const existingVanLetter = await prisma.cartaVan.findUnique({
      where: { id },
    });
    ...
    const camposAlterados: { campo: string; valor: any }[] = [];
    ...
    for (const [campo, valor] of Object.entries(mapeamentoCampos)) {
      const key = campo.split('.').pop() as keyof typeof existingVanLetter;
    ...
    const updatedVanLetter = await prisma.cartaVan.update({
      where: { id },
      data: {
